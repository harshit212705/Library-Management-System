...
<h1 data-title="All Users">All Users</h1>
...
<table>
  ...
  $sql = "SELECT * FROM user_credentials WHERE account_type = '$user' ORDER BY username LIMIT $start_from, ".$results_per_page;
  ...
  if ($total){
	  ...
	  while ($rows = fetch_data($result)){
		...
		echo "<tr><td>".$i."</td><td>".$rows['username']."</td><td>".$rows['email_id'].    "</td><td>".$rows['mobile_number']."</td><td>".$rows['gender']."</td><td>".$fine."</td></tr>";
         $i = $i + 1;		
	  }
	  echo "</table>";
  
	 $request = "SELECT * FROM user_credentials WHERE account_type = '$user'";
	 ...
	 for ($i=1; $i<=$total_pages; $i++) {  // print links for all pages
			...
			echo "<h2>";
			echo "<a href='all_users.php?page=".$i."'";
            echo ">".$i."&nbsp;&nbsp;&nbsp;&nbsp;"."</a>";
			echo "</h2>";
			}
			else{
